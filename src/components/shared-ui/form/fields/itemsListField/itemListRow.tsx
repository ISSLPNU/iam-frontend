...
export const ItemListRow = <T extends FieldValues, TItem>(
	{
		item,
		index,
		field,
	...
	if (onItemSelected) {
		return (
			<StyledItemListRow
				...
				onClick={() => {
					if (field.value) {
						field.value[index] = onItemSelected(item, index)
	...
	return (
			...
			<Modal<TItem>
				...
				onClose={(returnedValue, closeStatus) => {
					...
					if (closeStatus !== ModalCloseStatus.CANCELED) {
						field.value[index] = returnedValue;
						field.onChange(field.value)
					}
				}}
				render={modalRender}/>
			{onRemove && <RemoveModal<TItem>
          isOpen={isRemoveModalOpen && !field.disabled}
      ...
      />}
            ...
            <StyledRemoveButton $disabled={field.disabled || false}/>
...
interface IItemListRowProps<T extends FieldValues, TItem> extends ComponentProps<"div"> {
	item: TItem
	index: number
